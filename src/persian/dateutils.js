...
function pFormat(xd, format) {
  return toPersian(
    moment(typeof xd === 'string' ? xd : xd.toDate()).format(format),
  );
}
...
function pDateDay(xd) {
  ...
  if (isIntlSupported) {
    ...
    return (cache.dates[key] = new Intl.DateTimeFormat('fa-IR', options).format(
      xd.toDate(),
    ));
  }
...
function isSameMonth(a, b, jalali = false) {
  if (jalali) {
    ...
    a = pMoment(a);
    b = pMoment(b);
    return a.jYear() === b.jYear() && a.jMonth() === b.jMonth();
  }
...
function month(xd, jalali = false) {
  ...
  const firstDay = jalali
    ? toXd(pMoment(xd).startOf('jMonth'))
    : new XDate(year, month, 1, 0, 0, 0, true);
  const lastDay = jalali
    ? toXd(pMoment(xd).endOf('jMonth'))
    : new XDate(year, month, days, 0, 0, 0, true);
